...
if (!customElements.get('variant-picker')) {
  class VariantPicker extends HTMLElement {
    constructor() {
      ...
      this.productForm = this.section.querySelector(`#${this.dataset.formId}`);
    ...
    handleVariantChange(evt) {
      ...
      if(this.dataset.simpleMode) return
      ...
      this.dispatchEvent(new CustomEvent('on:variant:change', {
        ...
        detail: {
          ...
          formatted: this.data.formatted[this.variant.id].price,
        }
      }));

      if (purchaseOptions) {
        purchaseOptions.variant = this.variant;
        purchaseOptions.price = this.variant.price;
    ...
    updateVariantInput() {
      ...
      this.forms.forEach((form) => {
        if (window.purchaseOptions.is_bundle) return;
