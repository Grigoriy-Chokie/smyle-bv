  ...
  customElements.whenDefined('product-form').then(() => {
    class StickyAtc extends customElements.get('product-form') {
      constructor() {
        ...
        this.productData = JSON.parse(this.querySelector('[data-product-data]').innerHTML)
        ...
        document.addEventListener('sticky-atc:update', this.updateInfo.bind(this))

        this.updateImage();
      }

      updateInfo(event) {
        this.variantsQueue ??= new Set()
        this.mainImage = this.querySelector('.sticky-atc__image img')

        const { type, variantId } = event.detail

        if (type === 'add') {
          this.variantsQueue.delete(variantId)
          this.variantsQueue.add(variantId)
        } else {
          this.variantsQueue.delete(variantId)
        }

        const lastVariantId = Array.from(this.variantsQueue).at(-1)
        ...
        const galleryImage = this.section.querySelector(
          `.media-gallery [data-media-id="${mediaId}"] img`
        );

        if (!galleryImage) return

        this.mainImage.replaceWith(galleryImage.cloneNode(true))
      }
      ...
      static handleScroll() {
        ...
        this.classList.toggle(
          'sticky-atc--out',
          this.mainProductForm.getBoundingClientRect().bottom + topOffset > window.innerHeight
        );
