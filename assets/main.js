...
theme.suffixIds = (container, prefix) => {
  ...
  for (let i = 0; i < suffixCandidates.length; i += 1) {
    container.querySelectorAll(`[${suffixCandidates[i]}]:not([data-static-id])`).forEach((el) => el.setAttribute(suffixCandidates[i], el.getAttribute(suffixCandidates[i]) + prefix));
  }
};
...
theme.showQuickPopup = (message, origin) => {
  ...
  if (offsetRight > window.innerWidth) {
    ...
    popup.style.left = `${offsetLeft + marginLeft}px`;
  }
...
class DeferredMedia extends HTMLElement {
  ...
  attachEvents() {
    const controlsButton = this.querySelectorAll(".video-component__controll");
    ...
    ["play", "pause"].forEach(eventType => {
      video.addEventListener(eventType, () => {
        controlsButton.forEach(button => {
          button.classList.toggle("hidden", button.name !== eventType);
        });
      });
    });
  
    controlsButton.forEach(button => button.addEventListener("click", () => {
      if (button.name === "play") {
        video.pause();
...
function onPriceChanged() {
  ...
  const { purchase_type, product_type, quantity, price, onetime_prices, subscription_prices } = purchaseOptions;

  const onetimePrices = calculatePrices(quantity, price, onetime_prices, product_type, 'onetime');
  const subscriptionPrices = calculatePrices(quantity, price, subscription_prices, product_type, 'subscription');

  purchaseOptions.original_price = {
    onetime: onetimePrices.original,
    subscription: subscriptionPrices.original,
    current: purchase_type === 'subscription' ? subscriptionPrices.original : onetimePrices.original,
  };

  purchaseOptions.discounted_price = {
    onetime: Math.ceil(onetimePrices.discounted),
    subscription: Math.ceil(subscriptionPrices.discounted),
    current: Math.ceil(purchase_type === 'subscription' ? subscriptionPrices.discounted : onetimePrices.discounted),
  };

  updatePriceElements();

  function calculatePrices(quantity, price, discounts, productType, purchaseType) {
    const discountedPercent = extractDiscount(quantity, discounts);
    const discountedPackPrice = price - (price * discountedPercent / 100);

    const originalPrice = price * quantity;
    const discountedPrice =
      productType === 'Brush'
        ? calculateBrushDiscountedPrice(quantity, price, discounts)
        : discountedPackPrice * quantity;

    return { original: originalPrice, discounted: discountedPrice };
  }

  function extractDiscount(quantity, discounts) {
    if (!discounts || discounts.length === 0) return 0;

    const maxVolumeQuantity = parseInt(discounts[discounts.length - 1].split('--')[0].replace('+', ''), 10);
    const discountPerItem = discounts.find((discountValue) => {
      const discountedQuantity = parseInt(discountValue.split('--')[0].replace('+', ''), 10);
      return discountedQuantity === quantity || discountedQuantity === maxVolumeQuantity;
    })?.split('--')[1];

    return discountPerItem ? parseFloat(discountPerItem) : 0;
  }

  function calculateBrushDiscountedPrice(quantity, price, discounts) {
    ...
    discounts.forEach((discountItem, index) => {
      const isLastItem = index === discounts.length - 1;
      if (quantityAdded > 0 && !isLastItem) return;

      const discountPercent = parseFloat(discountItem.split('--')[1].replace('+', ''));
      const currentQuantity = isLastItem ? quantity - quantityAdded : 1;
      quantityAdded += currentQuantity;

      if (currentQuantity > 0) {
        const discountedPrice = (price - (price * discountPercent / 100)) * currentQuantity;
  ...
  function updatePriceElements() {
    const { original_price, discounted_price, quantity, quantity_step } = purchaseOptions;
    if (!original_price || !discounted_price) return;

    document.querySelectorAll('[data-original-price]').forEach((priceElement) => {
      if (priceElement.closest('product-duo') || priceElement.closest('product-block')) return;

      priceElement.innerText = window.Shopify.formatMoney(original_price.current);
      priceElement.style.display = discounted_price.current >= original_price.current ? 'none' : 'inline';
      if (priceElement.hasAttribute('data-subscription')) {
        priceElement.innerText = window.Shopify.formatMoney(original_price.subscription);
        priceElement.style.display = discounted_price.subscription >= original_price.subscription ? 'none' : 'inline';
      }
      if (priceElement.hasAttribute('data-onetime')) {
        priceElement.innerText = window.Shopify.formatMoney(original_price.onetime);
        priceElement.style.display = discounted_price.onetime >= original_price.onetime ? 'none' : 'inline';
      }
    });

    document.querySelectorAll('[data-discounted-price]').forEach((priceElement) => {
      if (priceElement.closest('product-duo') || priceElement.closest('product-block')) return;

      priceElement.innerText = window.Shopify.formatMoney(discounted_price.current);
      if (priceElement.hasAttribute('data-subscription')) {
        priceElement.innerText = window.Shopify.formatMoney(discounted_price.subscription);

        if (priceElement.hasAttribute('data-per-beg-price')) {
          const quantityPerMonth = quantity / quantity_step;

          priceElement.innerText = window.Shopify.formatMoney(discounted_price.subscription / quantity * quantityPerMonth)
        }
      }
      if (priceElement.hasAttribute('data-onetime')) {
        priceElement.innerText = window.Shopify.formatMoney(discounted_price.onetime);

        if (priceElement.hasAttribute('data-per-beg-price')) {
          const quantityPerMonth = quantity / quantity_step;
          
          priceElement.innerText = window.Shopify.formatMoney(discounted_price.onetime / quantity * quantityPerMonth)
        }
      }
      if (priceElement.hasAttribute('data-price')) {
        ...
        priceElement.innerText = window.Shopify.formatMoney(price * quantity);
      }
    });
  }
}

window.addEventListener('on:price:changed', onPriceChanged);
...
const CartForm = class extends HTMLElement {
  ...
  adjustItemQuantity(item, change) {
    ...
    if (typeof change.to !== 'undefined') {
      ...
      const step = +quantityInput.step;
      const hasStep = step > 1;
      const quantity = Math.abs(+quantityInput.value);
      const extraQuantity = quantity % step;

      if (hasStep && extraQuantity > 0) {
        const newQuantity = quantity - extraQuantity + step;
        quantityInput.value = newQuantity;
      }
    };
    ...
    this.adjustItemQuantityTimeout = setTimeout(() => {
      ...
      this.querySelectorAll('.cart-item__quantity-input:not([disabled])').forEach((el) => {
        const itemEl = el.closest('[data-merge-list-item]');
        const key = el.dataset.key;
        const quantity = parseInt(el.value, 10);
        const initialValue = el.dataset.initialValue;
  
        const propsJSON = itemEl.dataset.lineitemProperties;
        const props = propsJSON ? JSON.parse(propsJSON) : {};
  
        const productType = itemEl.dataset.productType || '';
        const subscriptionBundleId = props.subscription_bundle_id || null;

        const kitPropsJson = itemEl.dataset.kitProperties;
        const kitProps = kitPropsJson ? JSON.parse(kitPropsJson) : {};

        const kitId = kitProps.kit_id || null;
        const isAdditionalKit = kitProps.is_additional_product || null;
  
        updateParam.updates[key] = quantity;
        items.push({ key, quantity, productType, subscriptionBundleId, kitId, isAdditionalKit });
  
        if (el.value !== initialValue) {
          itemEl.classList.add('merge-item-refreshing');
        }
      });
      ...
      for (const item of items) {
        if (!item.subscriptionBundleId && !item.kitId) continue;
  
        if (item.subscriptionBundleId) {
          if (!bundleGroups[item.subscriptionBundleId]) {
            bundleGroups[item.subscriptionBundleId] = { brushhead: null, others: [] };
          }
    
          if (item.productType.toLowerCase().includes('brush-head')) {
            bundleGroups[item.subscriptionBundleId].brushhead = item;
          } else {
            bundleGroups[item.subscriptionBundleId].others.push(item);
          }
        }

        if (item.kitId) {
          if (!bundleGroups[item.kitId]) {
            bundleGroups[item.kitId] = { additional: null, main: [] };
          }
    
          if (item.isAdditionalKit) {
            bundleGroups[item.kitId].additional = item;
          } else {
            bundleGroups[item.kitId].main.push(item);
          }
        }
        
      }
  
      for (const id in bundleGroups) {
        const group = bundleGroups[id];
        if (group.brushhead) {
          const targetQty = group.others.reduce((sum, item) => sum + item.quantity, 0);
  
          if (group.brushhead && group.brushhead.quantity !== targetQty) {
            updateParam.updates[group.brushhead.key] = targetQty;
          }
        }

        if (group.additional) {
          const kitMainProductsQty = group.main.reduce((sum, item) => sum + item.quantity, 0);
  
          updateParam.updates[group.additional.key] = kitMainProductsQty > 0 ? 1 : 0;
        }
      }
...
const MainNavigation = class extends HTMLElement {
  ...
  connectedCallback() {
    ...
    // insert into page
    document.querySelector('.section-header').append(this.mobileDrawer);
    ...
    theme.addDelegateEventListener(this.mobileDrawer, 'click', '.navigation__tier-1 > .navigation__item > .navigation__children-toggle', (evt, delEl) => {
      ...
      delEl.nextElementSibling.style.top = `${Math.ceil(this.mobileDrawer.querySelector('.navigation__mobile-header').clientHeight + 16)}px`;
...
const MediaGallery = class extends HTMLElement {
  constructor() {
    ...
    this.variantImageChangeEnabled = this.variantPicker && this.hasAttribute('data-variant-image-change-enabled');
    this.mediaLists = [
  ...
  setFromVariantPicker() {
    if (this.mediaGroupingEnabled) {
      ...
      if (this.variantImageChangeEnabled && variant && variant.featured_media) {
   ...
   * @param {Event} event - variant change event dispatched by variant-picker
   */
  async onVariantChange(event) {
    if(this.hasAttribute('data-gallery-dynamic')){
      try {
        const fetchedSection = await (await fetch(`${this.productUrl}?variant=${event.detail.variant.id}&sections=${this.sectionId}`)).json()
        const [parsedGallery, parsedGalleryViewer] = new DOMParser().parseFromString(fetchedSection[this.sectionId], 'text/html').querySelectorAll('media-gallery, gallery-viewer');

        if(parsedGalleryViewer) document.querySelector(`gallery-viewer[data-section-id="${this.sectionId}"]`)?.replaceWith(parsedGalleryViewer);
        if(parsedGallery) this.replaceWith(parsedGallery);
      }catch (error) {
        console.log(error);
      }
    }
    ...
    if (this.mediaGroupingEnabled) {
      this.setActiveMediaGroup(this.getMediaGroupFromOptionSelectors(event));
    }

    if (this.variantImageChangeEnabled && event.detail.variant && event.detail.variant.featured_media) {
      this.setActiveMedia(event.detail.variant.featured_media.id, true);
...
const ProductForm = class extends HTMLElement {
  constructor() {
    ...
    this.form = this.querySelector(`#${this.dataset.formId}`) || this.querySelector('.js-product-form');
    if (this.form) {
      ...
      if (window.purchaseOptions) window.dispatchEvent(new CustomEvent('on:price:changed'));
    }
  }
  ...
  validateFormData(formData) {
    ...
    for (const [key, value] of Object.entries(rawData)) {
      ...
      if (match) {
        const [_, indexStr, property] = match;
        ...
        if (property.startsWith("properties")) {
          ...
          if (propMatch) {
            ...
            itemsMap.get(index).properties ??= {};
            itemsMap.get(index).properties[propKey] = value;
          }
        } else {
          itemsMap.get(index)[property] = value;
          if (property === 'selling_plan') {
            itemsMap.get(index)['[properties][_selling_plan_id]'] = value;
          }
        }
      } else {
        if (key == 'id' || key == 'quantity' || key == 'selling_plan' || key.includes('properties')) {
          if (!itemsMap.has('-1')) {
            itemsMap.set('-1', {});
          }
          ...
          if (key.startsWith("properties")) {
            ...
            if (propMatch) {
              const propKey = propMatch[1];
              itemsMap.get('-1').properties ??= {};
              itemsMap.get('-1').properties[propKey] = value;
            }
          } else {
            itemsMap.get('-1')[key] = value;
          }

          if (key == 'selling_plan') {
            itemsMap.get('-1')['properties[_selling_plan_id]'] = value;
          }
    ...
    //Delete no quantity items and options field without connected id
    itemsMap.forEach((item, index) => {
      if (typeof item.quantity !== 'undefined' && item.quantity == 0) itemsMap.delete(index);
      if (typeof item.properties !== 'undefined' && item.id == undefined) itemsMap.delete(index);
    })
        ...
        for (const [prop, val] of Object.entries(item)) {
          if (prop === 'properties') {
            for (const [subKey, subVal] of Object.entries(val)) {
              output[`items[${newIndex}][properties][${subKey}]`] = subVal;
            }
          } else {
            output[`items[${newIndex}][${prop}]`] = val;
          }
        }
      });
  ...
  async handleSubmit(evt) {
    ...
    if(window.purchaseOptions?.is_bundle) formData.append('properties[_main_product_url]', window.location.href);

    const validFormData = this.validateFormData(formData)
    ...
    try {
      ...
      if (document.querySelector('.template-cart')) {
        ...
        const hasAdditionalProduct = formData.get('items[31][properties][_kit_additional_product]');
        if (hasAdditionalProduct) {
          const cartForm = document.querySelector('cart-form');
          cartForm?.adjustItemQuantity(cartForm.querySelector('.cart-item'), { currentValue: true })
        }
...
if (!customElements.get('quantity-wrapper')) {
  class QuantityWrapper extends HTMLElement {
    connectedCallback() {
      this.decreaseButton = this.querySelector('[data-quantity="down"]');
      this.increaseButton = this.querySelector('[data-quantity="up"]');
      this.addEventListener('click', this.handleClick.bind(this));
    }
    ...
    handleClick(evt) {
      ...
      if (btn) {
        ...
        const newQuantity = Math.max(1, parseInt(input.value, 10) + change);
        if (input.max && input.max < newQuantity || input.min && input.min > newQuantity) return;
        ...
        const hasReachedMin = input.min && input.min == newQuantity;
        const hasReachedMax = input.max && input.max == newQuantity;
        
        this.decreaseButton?.toggleAttribute('disabled', hasReachedMin);
        this.increaseButton?.toggleAttribute('disabled', hasReachedMax);
      }
    }
  }
...
class QuantityRules extends HTMLElement {
  static TYPE_ONETIME = "onetime";
  static TYPE_SUBSCRIBE = "subscribe";

  constructor() {
      super();
      this.subscribeRulesData = this.getJson("subscribe-rules")
      this.subscribeRules = this.subscribeRulesData.rules;
      this.subscribeRulesMin = this.subscribeRules?.[0];
      this.subscribeRulesMax = this.subscribeRules?.[this.subscribeRules.length - 1];
      ...
      this.quantityLabel = this.querySelector('.quantity-subscribe__selector-text');

      this.oneTimeInputOptions = this.querySelectorAll("input[name='quantity-onetime-rules']")
      this.subscribeInputOptions = this.querySelectorAll("input[name='quantity-subscribe-rules']")

      this._typeSelector = this.dataset.type;
      this._quantity = this.determineInitialValue();

    document.querySelector('variant-picker')?.addEventListener('on:variant:change', this.variantChangeHandler.bind(this));
  }
  ...
  set quantityRule(quantity) {
    this._quantity = quantity;
    this.updateInputs();
    if (purchaseOptions) {
      purchaseOptions.quantity = this.quantityRule * this.subscribeRulesData.itemPerUnit;
      window.dispatchEvent(new CustomEvent('on:price:changed'));
    }
  }

  get quantityTypeSelector() {
      return this._typeSelector;
  }

  set quantityTypeSelector(type) {
      this._typeSelector = type;
      this.resetSelector(type);
  }
  ...
  connectedCallback() {
      ...
      this.oneTimeInputOptions.forEach(input => input.addEventListener("change", this.onQuantionInputChange.bind(this, input)))
      this.subscribeInputOptions.forEach(input => input.addEventListener("change", this.onQuantionInputChange.bind(this, input)))
  }

  variantChangeHandler(event){
    this.subscribeRulesData.productPrice = event.detail.variant.price

    this.updateSubscriptionSelector()
    this.updateOneTimePrices()
  }

  determineInitialValue() {
    if (this.quantityTypeSelector === QuantityRules.TYPE_ONETIME) {
      const selectedInput = [...this.oneTimeInputOptions].find(input => input.checked) || [...this.oneTimeInputOptions][0];
      return selectedInput.value
    }

    if (this.quantityTypeSelector === QuantityRules.TYPE_SUBSCRIBE) {
      return this.subscribeRulesMin.quantity / this.subscribeRulesData.itemPerUnit
    }
  ...
  updateInputs() {
      ...
      inputs.forEach(input => input.value = this.quantityRule * this.subscribeRulesData.itemPerUnit);
  }

  updateOneTimePrices(){
    this.querySelectorAll('[data-discount]').forEach(qunatityBox => {
      const {discount, quantity} = qunatityBox.dataset

      const price = qunatityBox.querySelector('.onetime-label__price-money')
      const priceMonthly = qunatityBox.querySelector('[data-price-monthly]')


      if(price) price.textContent = window.Shopify.formatMoney(this.subscribeRulesData.productPrice * quantity * Math.abs(1 - discount))
      if(priceMonthly) priceMonthly.textContent = window.Shopify.formatMoney(this.subscribeRulesData.productPrice * Math.abs(1 - discount))
    })
  }

  updateSubscriptionSelector(quantity) {
      quantity = (quantity || this.quantityRule) * this.subscribeRulesData.itemPerUnit
      const { quantity: minQuantity, price: minPrice } = this.subscribeRulesMin;
      const { quantity: maxQuantity } = this.subscribeRulesMax;

      const determineQuantityRules = () => {
          let currentRule;

          if (Number(quantity) <= Number(minQuantity)) {
              currentRule = this.subscribeRulesMin;
          } else if (Number(quantity) >= Number(maxQuantity)) {
              currentRule = this.subscribeRulesMax;
          } else {
              currentRule = this.subscribeRules.find(({ quantity: currentQuantity }, index) =>
                Number(currentQuantity) <= Number(quantity) &&
                Number(this.subscribeRules[index + 1]?.quantity || 100) > Number(quantity));
          }
          return currentRule
      }

      const { discount, price_month_money, quantity: currentQuantity } = determineQuantityRules();
      const fullCurrentPrice = this.subscribeRulesData.productPrice * currentQuantity * Math.abs(1 - discount);

      const quantityWrapper = this.querySelector(".quantity-subscribe__wrapper");
      const quantitySaveBadge = this.querySelector(".quantity-subscribe__badges-save");
      const quantitySaveBadgeText = quantitySaveBadge.querySelector("small");
      const quantityPrice = this.querySelector(".quantity-subscribe__prices-price");
      const quantityMonth = this.querySelector(".quantity-subscribe__prices-month");

      quantityWrapper?.classList.toggle("quantity-subscribe__wrapper--active", Number(currentQuantity) !== Number(minQuantity));
      quantitySaveBadge?.classList.toggle("quantity-subscribe__badges-save--disabled", discount == 0);

      if (quantityPrice) quantityPrice.textContent = window.Shopify.formatMoney(Math.ceil(fullCurrentPrice));
      if (quantityMonth) quantityMonth.textContent = window.Shopify.formatMoney(Math.ceil(fullCurrentPrice / quantity));

      if (discount > 0 && quantitySaveBadgeText) quantitySaveBadgeText.textContent = discount * 100;

  }

  resetSelector(type) {
      this.dataset.type = type;
      if (type === QuantityRules.TYPE_ONETIME) {
          const selectedInput = [...this.oneTimeInputOptions].find(input => input.checked) || [...this.oneTimeInputOptions][0];
          this.quantityRule = selectedInput.value / this.subscribeRulesData.itemPerUnit
          if (this.quantityInput) this.quantityInput.value = selectedInput.value
      }
      if (type === QuantityRules.TYPE_SUBSCRIBE) {
          this.quantityRule = this.subscribeRulesMin.quantity / this.subscribeRulesData.itemPerUnit;
          if (this.quantityInput) this.quantityInput.value = this.subscribeRulesMin.quantity / this.subscribeRulesData.itemPerUnit;

          this.updateSubscriptionSelector(this.subscribeRulesMin.quantity / this.subscribeRulesData.itemPerUnit)
      }
  }

  updateSubscriptionQuantityLabel() {
    if (!this.quantityLabel) return;

    const { packOneText, packMoreText } = this.quantityLabel.dataset;
    if (!packOneText || !packMoreText) return;

    this.quantityLabel.innerHTML = this.quantityRule > 1 ? packMoreText : packOneText;
  }
  ...
  onQuantionInputChange(input) {
      this.quantityRule = input.value / this.subscribeRulesData.itemPerUnit || 1
  }

  onQuantityChange() {
      this.quantityRule = this.quantityInput.value || 1
      this.updateSubscriptionSelector(this.quantityRule);
      this.updateSubscriptionQuantityLabel();
  }

  getJson(idSelector) {
      const script = document.getElementById(idSelector);
      if (!script || script.type !== "application/json") return [];
      try {
          return JSON.parse(script.textContent);
      } catch (error) {
          console.error(`Error parsing JSON from #${idSelector}:`, error);
          return [];
      }
  }
...
window.customElements.define('quantity-rules', QuantityRules);